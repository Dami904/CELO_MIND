...
import { makeOk, makeErr , resolveNetwork } from "@celomind/shared";
  ...
  app.get("/api/dashboard/metrics", async (req, reply) => {
    try {
      const [celoRes, tvlRes, trendingRes] = await Promise.allSettled([
        fetch("https://api.coingecko.com/api/v3/simple/price?ids=celo&vs_currencies=usd&include_24hr_change=true").then((r) => r.json()),
        fetch("https://api.llama.fi/v2/chains").then((r) => r.json()),
        fetch("https://api.geckoterminal.com/api/v2/networks/celo/trending_pools?page=1").then((r) => r.json()),
      ]);
