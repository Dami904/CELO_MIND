...
// ── Quick-start chips shown in thread before first user message ────────────────
const QUICK_PROMPTS = [
  { label: 'Check my balance',         icon: '💰' },
  { label: 'What is the CELO price?',  icon: '📈' },
  { label: 'Show top DeFi pools',      icon: '🌊' },
  { label: 'Is this contract safe?',   icon: '🛡️' },
  { label: 'Swap CELO for cUSD',       icon: '🔄' },
  { label: 'Show whale activity',      icon: '🐋' },
];
    ...
    <div className="flex" style={{ height: 'calc(100vh - 64px)' }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0 bg-stone-50 dark:bg-[#0F0E0C]">
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-6 flex flex-col gap-5 max-w-2xl w-full mx-auto relative">
          ...
          {/* Quick-start chips — only visible before first user message */}
                ...
                {QUICK_PROMPTS.map((p) => (
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-left text-sm
                      ...
                      hover:border-[#FCBE00] dark:hover:border-amber-500/50
                      hover:bg-[#FFF8D6] dark:hover:bg-[#2A2510]
